...
if __name__ == "__main__":
    ...
    image = cv2.imread('challenging-60/28_outdoor_hazy.jpg', cv2.COLOR_BGR2RGB)
    image = cv2.resize(image, (640, 480))

    GT = cv2.imread('challenging-60/28_outdoor_GT.jpg', cv2.COLOR_BGR2RGB)
    GT = cv2.resize(GT, (640, 480))
    image_set = [[] for _ in range(3)]
    for c in range(3):
        ...
        idft2d = gamma(idft2d, 0.5)
        image_set[c] = idft2d

    dehazed_img = cv2.merge(image_set)

    image = SIFT(image, GT)
    dehazed_img = SIFT(dehazed_img, GT)

    cv2.imshow('before', image)
    cv2.imshow('after', dehazed_img)
    cv2.waitKey()
    ...
    # ax2.imshow(dehazed_img), ax2.axis('off')
    #
    # plt.show()
